...
process trimmomatic{
	...
	output:
	tuple sampleID, file("${sampleID}/*_paired.fq.gz") into trimmomatic_output 
...
process spades_no_trim{
	...
	output:
	tuple sampleID, file("${sampleID}/${sampleID}*") into spades_output
...
process skesa_no_trim{
	...
	output:
	tuple sampleID, file("${sampleID}/${sampleID}*") into skesa_output_no_trim
...
process pilon_post_spades_no_trim {
	...
	publishDir './Results/No_trimming/Pilon/SPAdes', mode: 'copy'
...
process pilon_post_skesa_no_trim {
	...
	publishDir './Results/No_trimming/Pilon/SKESA', mode: 'copy'
	
	input:
	tuple sampleID, file(scaffold), file(reads) from skesa_raw_data_join
	
	output:
	tuple sampleID, file("${sampleID}/${sampleID}*") into pilon_output_skesa_no_trim
...
process quast_no_trimming{
	...
	publishDir './Results/No_trimming/Quast/No_improvement', mode: 'copy'
...
process quast_no_trimming_improved{
	...
	output:
	file "SPAdes/${sampleID}/*" into quast_output_spades_improved_no_trim
	file "SKESA/${sampleID_skesa}/*" into quast_output_skesa_improved_no_trim
