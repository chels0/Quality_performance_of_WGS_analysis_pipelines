...
for files in combos:
    ...
    for i in range(len(indices2)):
        ...
        #otherwise only add first 3 indices of indices2 list
            ...
            indices.append(indices2[i][0]+'_'+indices2[i][1]+'_'+indices2[i][2])
    ...
    test2 = df_chewbbaca.compare(df2_chewbbaca, align_axis=1).rename(columns={'self': filename, 'other': filename2}, level=-1)
    test2 = test2.fillna('')
    ...
    filename3 = filename.split('_results')
